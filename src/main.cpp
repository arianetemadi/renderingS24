    ...
    for (int x = 0; x < size.x(); ++x) {
      for (uint32_t i = 0; i < 1; ++i) {
        Point2f pixelSample =
            ...
            Point2f(0.5f, 0.5f);  // go through the centre of the pixel
        pixelSample.x() = std::min(
            pixelSample.x(), std::nextafter(float(x + offset.x() + 1), 0.f));
        pixelSample.y() = std::min(
            pixelSample.y(), std::nextafter(float(y + offset.y() + 1), 0.f));
...
static void render(Scene *scene, const std::string &filename) {
  ...
  std::thread render_thread([&] {
    ...
    std::cout << "# benchmark # Rendering took: "
...
int main(int argc, char **argv) {
  ...
  if (exrName != "" && sceneName != "") {
    cerr << "Both .xml and .exr files were provided. Please only provide one "
            "of them."
         << endl;
    return -1;
